...
export default class AddDocumenttype extends Component {
  ...
  componentDidMount(){
    if(this.state.doctype !== " "){
        axios.get('/document-type/find/'+this.state.doctype).then(response => {
          this.setState({
  ...
  addDoctype(type){
    axios.post(`/document-type/create`, type ,
    {
  ...
  editDoctype(type){
    ...
    console.log(data)
    axios.patch(`/document-type/update/${this.state.doctype}`, data ,
    {
      'Content-type':'application/json'
    }).then(res => {
